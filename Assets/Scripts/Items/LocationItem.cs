...
public class LocationItem : MonoBehaviour {
    ...
    internal void Init(LocationsFinderPanel locationResultsPanel, string placeId, string name, string sessionToken) {
    ...
    private IEnumerator LoadLocationDetails(Action<double, double> OnLoaded) {
        var loaded = new UnityWebRequest(directionsURL + "place_id=" + location.PlaceId + "&fields=geometry&key=" + Program.googleKey + "&sessiontoken=" + sessionToken);
