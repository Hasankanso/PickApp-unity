...
public class AddRidePanel : Panel
{
  ...
  public void Init()
  {
    ...
    if (Program.User != null&&Program.User.IsDriver) {
        driver = (Driver)Program.User;
    }
  }
  ...
  public void OpenMyCarsPicker()
  {
    ...
    if (ValidateSecondView())
    {
      ...
      ride = new Ride(driver, null, fromL, toL, comment.text.text, price.text.text, Program.CountryInformations,
  ...
  private bool ValidateSecondView()
  {
    ...
    if (stopTimeCheckBox.isOn)
    {
      ...
      if (int.Parse(stopTimeField.text.text) < 0)
      {
        stopTimeField.Error();
        valid = false;
      ...
      if (int.Parse(stopTimeField.text.text) > 120)
      {
        stopTimeField.Error();
        valid = false;
